...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NPM_TOKEN_TEST_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: NpmTokenTest) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NPM_TOKEN_TEST_LOG'] = 'debug';
      const client = new NpmTokenTest({ logger: logger, username: 'My Username', password: 'My Password' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NPM_TOKEN_TEST_LOG'] = 'not a log level';
      const client = new NpmTokenTest({ logger: logger, username: 'My Username', password: 'My Password' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NPM_TOKEN_TEST_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['NPM_TOKEN_TEST_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['NPM_TOKEN_TEST_BASE_URL'] = 'https://example.com/from_env';
      const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['NPM_TOKEN_TEST_BASE_URL'] = ''; // empty
      const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://todo-ninja-ziix.onrender.com');
    });

    test('blank env variable', () => {
      process.env['NPM_TOKEN_TEST_BASE_URL'] = '  '; // blank
      const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
      expect(client.baseURL).toEqual('https://todo-ninja-ziix.onrender.com');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['NPM_TOKEN_TEST_BASE_URL'] = 'http://localhost:5000/env';
      const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new NpmTokenTest({ maxRetries: 4, username: 'My Username', password: 'My Password' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['NPM_TOKEN_TEST_USERNAME'] = 'My Username';
    process.env['NPM_TOKEN_TEST_PASSWORD'] = 'My Password';
    const client = new NpmTokenTest();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['NPM_TOKEN_TEST_USERNAME'] = 'another My Username';
    process.env['NPM_TOKEN_TEST_PASSWORD'] = 'another My Password';
    const client = new NpmTokenTest({ username: 'My Username', password: 'My Password' });
    expect(client.username).toBe('My Username');
    expect(client.password).toBe('My Password');
  });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new NpmTokenTest({ username: 'My Username', password: 'My Password', fetch: testFetch });
